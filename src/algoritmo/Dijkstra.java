...
public class Dijkstra {
	...
	public double[][] shortPathTotalParada(Vertice v, int tamanhoLista) {
		//lista de elementos ja acessados
		ArrayList<Boolean> lista = new ArrayList<>();
		//saida final: custo e elemento anterior
		double[][] saida = new double[tamanhoLista][2];
		...
		while (!pilha.isEmpty()) {
			...
			//para cada adjacencia
			for (Aresta a : vertice.getListaAresta()) {
				//se a distancia for maior que a distancia maxima a ser percorida sem paradas
				//ignora a adjacencia
				if (a.getDistancia() > new Veiculo().getDistanciaMaximaSemParada()) {
					//distancia inalcançavel
				} else {
					Vertice adj = a.otherSide(vertice);
					//peso vertice adjacente
					double pesoA = saida[adj.getKey()][0];
					...
					pilha.push(adj);
				}//fim if distancia maxima
			}//fim for each
		}
		return saida;
	}//fim shot path total parada
